...
class SodiumPlus {
     ...
     * @param {string|Buffer} plaintext
     * @param {Buffer} nonce
     * @param {CryptographyKey} myPrivateKey
     * @param {CryptographyKey} theirPublicKey
     * @return {Promise<Buffer>}
     */
    async crypto_box(plaintext, nonce, myPrivateKey, theirPublicKey) {
        if (!(myPrivateKey instanceof CryptographyKey)) {
            throw new TypeError('Argument 3 must be an instance of CryptographyKey');
        }
        if (!(theirPublicKey instanceof CryptographyKey)) {
            throw new TypeError('Argument 4 must be an instance of CryptographyKey');
        }
        ...
        return this.backend.crypto_box(
            plaintext,
            nonce,
            await this.crypto_box_keypair_from_secretkey_and_publickey(myPrivateKey, theirPublicKey)
        );
    }
    ...
    async crypto_box_keypair_from_secretkey_and_publickey(sKey, pKey) {
        if (!(sKey instanceof CryptographyKey)) {
            throw new TypeError('Argument 1 must be an instance of CryptographyKey');
        }
        if (!(pKey instanceof CryptographyKey)) {
            throw new TypeError('Argument 2 must be an instance of CryptographyKey');
        }
